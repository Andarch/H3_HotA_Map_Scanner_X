...
function DisplayObjCountZoneTable($table)
{
    ...
    foreach ($table->objects as $obj) {
        $objcomboid = in_array($table->category, [OBJ_CATEGORY::NEUTRAL_DWELLINGS_1, OBJ_CATEGORY::NEUTRAL_DWELLINGS_2])
            ? $obj["truecomboid"]
            : $obj["comboid"];
        $objname = $obj["name"];
        $x = $obj["pos"]->x;
        ...
        if ($zone) {
            ...
            echo "Error: Zone color not found for " .
                $objcomboid .
                " " .
                $objname .
                " at [" .
    ...
    if (!empty($table->customOrder)) {
        ...
        uasort($flatObjCountPlayers, function ($a, $b) use ($order) {
            ...
            } elseif (array_key_exists("comboid", $a) && array_key_exists("comboid", $b)) {
    ...
    echo '<table class="table-small" style="margin-bottom: 3em !important;">
			<thead>
				<tr>
					<th colspan="100" class="ac table__title-bar--small">' .
        $table->category .
        '</th>
				</tr>';
