...
class H3MAPSCAN
{
    ...
    {
        return in_array($this->version, [$this::ROE, $this::AB, $this::SOD, $this::WOG]) ||
            ($this->version == $this::HOTA && $this->hota_subrev <= $this::HOTA_SUBREV9);
    }
    ...
    private function SaveMap()
    {
        ...
        $sql =
            "INSERT INTO heroes3_maps (heroes, campaign, id_campaign, `mapfile`, `mapdir`, `mapname`, `author`, `language`, `mapdesc`, `version`, `subversion`,
    ...
    public function ReadMap()
    {
        ...
        if ($this->printoutput && $this->webmode) {
            ...
            new H3MAPSCAN_PRINT($this, $section);
        }
    ...
    public function ReadMapEx()
    {
        ...
        if ($this->CheckVersion() == false) {
            throw new Exception(
                '<div class="content">Unknown version=' .
                $this->version .
                ", subrev=" .
                $this->hota_subrev .
                ". Possibly a campaign file or not a map (" .
                $this->mapfile .
                ")</div>",
            );
        }
    ...
    private function ReadPlayersData()
    {
        ...
        for ($i = 0; $i < PLAYERSNUM; $i++) {
            ...
            $this->players[$i]["townpos"]          = new MapCoords();
            $this->players[$i]["RandomHero"]       = "";
            $this->players[$i]["StartingHeroID"]   = OBJECT_INVALID;
            $this->players[$i]["StartingHeroFace"] = OBJECT_INVALID;
            $this->players[$i]["StartingHeroName"] = EMPTY_DATA;
            $this->players[$i]["HeroIDs"]          = [];
            $this->players[$i]["HeroNames"]        = [];
            ...
            if ($this->players[$i]["StartingHeroID"] != HNONE) {
                $this->players[$i]["StartingHeroFace"] = $this->br->ReadUint8();
                $this->players[$i]["StartingHeroName"] = $this->ReadString();
                if (!$this->players[$i]["StartingHeroName"]) {
                    $this->players[$i]["StartingHeroName"] = $this->GetHeroById($this->players[$i]["StartingHeroID"]);
                }
                ...
                for ($j = 0; $j < $herocount; $j++) {
                    ...
                    $this->players[$i]["HeroIDs"][]   = $heroid;
                    $this->players[$i]["HeroNames"][] = $heroname;
                }
            } else {
                $this->br->SkipBytes(1);
        ...
        switch ($this->victoryCond["type"]) {
            ...
            case VICTORY::ACCCREATURES: // 01 - Accumulate creatures
                ...
                $this->victoryInfo =
                    ...
                    $this->victoryCond["unit"] .
                    ", count " .
                    comma($this->victoryCond["unit_count"]);
                break;
            case VICTORY::ACCRESOURCES: // 02 - Accumulate resources
                ...
                $this->victoryInfo =
                    ...
                    $this->GetResourceById($this->victoryCond["resource"]) .
                    ", count: " .
                    comma($this->victoryCond["resource_count"]);
                break;
            case VICTORY::UPGRADETOWN: // 03 - Upgrade a specific town
                $this->victoryCond["name"] = "Upgrade a specific town";
                $this->victoryCond["coor"] = new MapCoords(
                    $this->br->ReadUint8(),
                    $this->br->ReadUint8(),
                    $this->br->ReadUint8(),
                );
                ...
                $this->victoryInfo =
                    "Upgrade a specific town at " .
                    $this->victoryCond["coor"]->GetCoords() .
                    " to " .
                    $this->victoryCond["hall_lvl"] .
                    " and " .
                    $this->victoryCond["castle_lvl"];
                break;
            ...
            case VICTORY::TRANSPORTART: // 0A - Transport a specific artifact
                ...
                $this->victoryInfo =
                    "Transport " .
                    $this->GetArtifactById($this->victoryCond["art"]) .
                    " to town at " .
                    $this->victoryCond["coor"]->GetCoords();
                break;
    ...
    private function LossCondition()
    {
        ...
        switch ($this->lossCond["type"]) {
            ...
            case LOSS::TOWN: // 00 - Lose a specific town
                ...
                $this->lossCond["coor"] = new MapCoords(
                    $this->br->ReadUint8(),
                    $this->br->ReadUint8(),
                    $this->br->ReadUint8(),
                );
        ...
        for ($i = 0; $i < SPELL_BYTE; $i++) {
            ...
            for ($n = 0; $n < 8; $n++) {
                if (($byte & (1 << $n)) != 0) {
                    ...
                    $spells[]            = $spell;
                    $this->spells_list[] = new ListObject(
                        $spell,
                        $this->curcoor,
                        $this->curobjtype,
                        OWNERNONE,
    ...
    private function ReadObjects()
    {
        ...
        $this->zoneowners_img_g = file_exists($zoneowners_img_g_path)
            ? imagecreatefrompng($zoneowners_img_g_path)
            : null;
        $this->zoneowners_img_u = file_exists($zoneowners_img_u_path)
            ? imagecreatefrompng($zoneowners_img_u_path)
            : null;
        $this->has_zone_images  =
            $this->zonetypes_img_g && $this->zonetypes_img_u && $this->zoneowners_img_g && $this->zoneowners_img_u
            ? true
            : false;
        ...
        for ($i = 0; $i < $this->objectsNum; $i++) {
            ...
            if (array_key_exists($obj["defnum"], $this->objTemplates)) {
                ...
                if (!in_array($obj["id"], $this->CS->OmittedObjectsZones)) {
                    ...
                    if ($this->has_zone_images) {
                        ...
                        [$obj["zone_type"], $obj["zone_owner"]] = $this->GetZone($obj["posoffset"]);

                        if (
                            ($obj["id"] == OBJECTS::SHIPWRECK ||
                                $obj["id"] == OBJECTS::PRISON ||
                                $obj["id"] == OBJECTS::DWELLING_NORMAL ||
                                $obj["id"] == OBJECTS::DWELLING_MULTI) &&
                        ...
                        if (
                            $obj["id"] == OBJECTS::MISC_OBJECTS_3 &&
                            $obj["subid"] == MISC_OBJECTS_3::QUEST_GATE &&
                            (in_array($obj["zone_type"], $ERROR_TYPES) || in_array($obj["zone_owner"], $ERROR_TYPES))
                        ) {
            ...
            if ($obj["id"] < 0) {
                throw new Exception(
                    '<div class="content">Invalid object ID ' .
                    $obj["id"] .
                    " - " .
                    $this->GetObjectNameById($obj["id"]) .
                    "  $x, $y, $z. Possibly a read error (" .
                    $this->mapfile .
                    ")</div",
                );
            }
            ...
            switch ($obj["id"]) {
                ...
                case OBJECTS::PANDORAS_BOX:
                    ...
                    if ($obj["id"] == OBJECTS::EVENT) {
                        ...
                        $event["removeAfterVisit"] = $event["removeAfterVisit"] == 1 ? "No repeat" : "Repeat";
                        ...
                        } elseif ($event["humanActivate"] == 1) {
                            $event["humanOrAi"] = "Human";
                        } elseif ($event["computerActivate"] == 1) {
                            $event["humanOrAi"] = "AI";
                        }
                    } else {
                        //pandora
                ...
                case OBJECTS::RANDOM_MONSTER_L7:
                    ...
                    $monster["name"] =
                        $obj["id"] == OBJECTS::MONSTER ? $this->GetCreatureById($obj["subid"]) : $obj["objname"];
                    $monster["pos"] = $obj["pos"];
                    if ($this->version > $this::ROE) {
                ...
                case OBJECTS::GARRISON_VERTICAL:
                    ...
                    $this->garrisons_list[] = new ListObject(
                        $name,
                        $obj["pos"],
                        "Map",
                        $stack["owner"],
                ...
                case OBJECTS::TOWN:
                    ...
                    foreach ($this->players as &$player) {
                        ...
                        if (
                            $player["townpos"]->x == $obj["pos"]->x &&
                            $player["townpos"]->y == $obj["pos"]->y &&
                            $player["townpos"]->z == $obj["pos"]->z
                        ) {
            ...
            if ($this->buildMapImage && $obj["id"] != OBJECT_INVALID) {
                ...
                foreach ($this->objTemplates[$obj["defnum"]]->tiles as $iy => $col) {
                    //y-axis of object tiles
                    foreach ($col as $ix => $tilemask) {
                        //x-axis of object tiles
                        ...
                        if (
                            $this->terrain[$z][$my][$mx]->owner != OWNERNONE ||
                            $this->terrain[$z][$my][$mx]->special != MAPSPECIAL::NONE
                        ) {
                            continue;
                        }
    ...
    private function ProcessPlayerObjectCount(
        $objcategory,
        $objid,
        $objsubid,
        $objcomboid,
    ...
    public function CreateRewardContents($event)
    {
        ...
        if (($event["move_bonus_type"] == 0 && $event["move_bonus_value"] > 0) || $event["move_bonus_type"] != 0) {
            if ($event["move_bonus_type"] != 1 && $event["move_bonus_value"] > 0) {
                $sign = "+";
            } elseif ($event["move_bonus_type"] == 1) {
                $sign = "-";
            } else {
                $sign = "";
            }
            switch ($event["move_bonus_type"]) {
                ...
                case 4: // Replenish
                    $content[1][] =
                        $sign .
                        comma($event["move_bonus_value"]) .
                        " Movement Points (" .
                        $event["move_bonus_type_name"] .
                        ")";
                    break;
                ...
                case 3: // Set
                    $content[1][] =
                        $event["move_bonus_type_name"] . " Movement Points to " . comma($event["move_bonus_value"]);
                    break;
            }
        }
    ...
    private function ReadTown()
    {
        ...
        if ($this->version > $this::ROE) {
            for ($i = 0; $i < SPELL_BYTE; $i++) {
                ...
                for ($s = 0; $s < 8; $s++) {
                    if (($spellb >> $s) & 0x01) {
                        //add obligatory spell even if it's banned on a map
                        $spellid = $i * 8 + $s;
        ...
        for ($e = 0; $e < $town["eventsnum"]; $e++) {
            ...
            if ($event["human"] == 1 && $event["computerAffected"] == 1) {
                $event["humanOrAi"] = "Human / AI";
            } elseif ($event["human"] == 1) {
                $event["humanOrAi"] = "Human";
            } elseif ($event["computerAffected"] == 1) {
                $event["humanOrAi"] = "AI";
            }
    ...
    private function ReadEvents()
    {
        ...
        for ($i = 0; $i < $numberOfEvents; $i++) {
            ...
            if ($event["humanAble"] == 1 && $event["aiAble"] == 1) {
                $event["humanOrAi"] = "Human / AI";
            } elseif ($event["humanAble"] == 1) {
                $event["humanOrAi"] = "Human";
            } elseif ($event["aiAble"] == 1) {
                $event["humanOrAi"] = "AI";
            }
            ...
            if ($this->hota_subrev >= $this::HOTA_SUBREV7) {
                $event["difficulties"] = $this->br->ReadUint32();
            }

            $event["type"] = $this->br->ReadUint8();
    ...
    private function ParseFinish()
    {
        ...
        foreach ($this->heroes_list as $hero_idx => $hero) {
            foreach ($this->towns_list as $town) {
                [$x, $y, $z] = [$hero["pos"]->x - 1, $hero["pos"]->y, $hero["pos"]->z];
                if (
                    $x == $town["pos"]->x &&
                    $y == $town["pos"]->y &&
                    $z == $town["pos"]->z
                    ...
                    if ($this->has_zone_images) {
                        $zone = $this->GetZone($this->heroes_list[$hero_idx]["pos"]);

                        $this->heroes_list[$hero_idx]["zone_type"]  = $zone[0];
                        $this->heroes_list[$hero_idx]["zone_owner"] = $zone[1];
                    }
                }
            }
        }

        //update hero names and portraits
        foreach ($this->templateHeroes as $k => $templateHero) {
            foreach ($this->heroes_list as $l => $mapHero) {
                if ($templateHero["id"] == $mapHero["data"]["subid"]) {
                    ...
                    if (
                        $mapHero["data"]["mapHeroName"] === $templateHero["defName"] &&
                        $templateHero["templateHeroName"] !== $templateHero["defName"]
                    ) {
            ...
            for ($i = 0; $i < PLAYERSNUM; $i++) {
                if ($templateHero["id"] == $this->players[$i]["StartingHeroID"]) {
                    $this->players[$i]["StartingHeroName"] = $this->templateHeroes[$k]["mapHeroName"];
                }
                for ($j = 0; $j < count($this->players[$i]["HeroIDs"]); $j++) {
                    if ($templateHero["id"] == $this->players[$i]["HeroIDs"][$j]) {
                        $this->players[$i]["HeroNames"][$j] = $this->templateHeroes[$k]["mapHeroName"];
                    }
                }
            }
        }
        ...
        switch ($this->victoryCond["type"]) {
            ...
            case VICTORY::BUILDGRAIL:
                $name = $this->GetMapObjectByPos(MAPOBJECTS::TOWN, $this->victoryCond["coor"]);
                $this->victoryInfo =
                    "Build the grail structure at town " . $name . " " . $this->victoryCond["coor"]->GetCoords();
                break;
    ...
    private function GetVersionName()
    {
        switch ($this->version) {
            case $this::ROE:
                $this->versionname = "RoE";
                break;
            case $this::AB:
                $this->versionname = "AB";
                break;
            case $this::SOD:
                $this->versionname = "SoD";
                break;
            case $this::WOG:
                $this->versionname = "WoG";
                break;
            case $this::HOTA:
    ...
    private function GetMapSize()
    {
        switch ($this->map_size) {
            ...
            case 108:
                ...
                break;
            case 144:
                $this->map_sizename = "XL";
                break;
            case 180:
                $this->map_sizename = "H";
                break;
            case 216:
                $this->map_sizename = "XH";
                break;
            case 252:
                $this->map_sizename = "G";
                break;
    ...
    private function GetDifficulty()
    {
        switch ($this->map_diff) {
            case 0:
                $this->map_diffname = "Easy";
                break;
            case 1:
                $this->map_diffname = "Normal";
                break;
            case 2:
                $this->map_diffname = "Hard";
                break;
            case 3:
                $this->map_diffname = "Expert";
                break;
            case 4:
                $this->map_diffname = "Impossible";
                break;
            default:
                $this->map_diffname = "?";
                break;
    ...
    */
    ...
    private function GetCoordsOffset(MapCoords $pos, int $id, int $subid): MapCoords
    {
        ...
        $CONDITIONAL_X_MINUS_2 = ["16-22", "16-26"];

        $OBJS_Y_OFFSET_MINUS_1 = ["219-0", "219-1", GARRISON_COMBOID, AMGARRISON_COMBOID];

        $CONDITIONAL_Y_OFFSET_MINUS_1 = [GARRISON_COMBOID, AMGARRISON_COMBOID];
        ...
        $should_apply_minus1 = function (string $comboid, $id) use ($CONDITIONAL_X_MINUS_1): bool {
            ...
            // HotA Visitable 1
            $hota_vis1 = ["144-2", "144-5", "144-6", "144-9", "144-10", "144-12"];
    ...
    private function GetZone(MapCoords $pos): array
    {
        ...
        $get_pixel_rgb = function ($img_g, $img_u) use ($x, $y, $z) {
            ...
            if ($rgba["alpha"] == 127) {
                // In PHP GD, alpha 127 is fully transparent
                return [null, "Void"];
            }
        ...
        [$rgb_types, $error_types] = $get_pixel_rgb($this->zonetypes_img_g, $this->zonetypes_img_u);
        $zone_type                 = $error_types ?: $this->CS->ZoneTypes[implode(",", $rgb_types)] ?? "Unknown";

        [$rgb_colors, $error_colors] = $get_pixel_rgb($this->zoneowners_img_g, $this->zoneowners_img_u);
    ...
    private function IsGZIP()
    {
        ...
        $header = end($header);
        if ($header == 0x00088b1f || $header == 0x08088b1f) {
            //possibly also 0x08088b1f
            $this->isGzip = true;
        } else {
            $this->isGzip = false;
        }
